...
gulp.task('build:css', () => {
  return gulp.src('src/anim-fx.css')
    .pipe(maps('init'))
    .pipe(autoprefixer())
    .pipe(maps('write', '.'))
    .pipe(gulp.dest('dist'));
...
gulp.task('build:js', () => {
  return gulp.src('src/jquery.anim-fx.js')
    .pipe(maps('init'))
    .pipe(rollup({
...
gulp.task('build', gulp.series('dev', () => {
  const stream1 = gulp.src('dist/anim-fx.css')
    .pipe(maps('init'))
    .pipe(cleanCSS())
  ...
  const stream2 = gulp.src('dist/jquery.anim-fx.js')
    .pipe(maps('init'))
    .pipe(uglify())
    .pipe(rename({ suffix: '.min' }))
    .pipe(maps('write', '.'))
